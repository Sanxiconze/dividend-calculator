...
- **真实股息率**：TTM 总额法计算（近12个月已除权），避免转送股带来的偏差
...
```
真实股息率TTM = 近12个月已除权现金分红总额 / 当前总市值
```

只统计**已实施除权**（除权除息日落在近 12 个月内）的现金分红；「预披露」方案不参与统计。
...
| 最新完整财年 | 半年报分红（9-12月除权）会被丢弃，若年报分红未除权则整体缺失 |
| **总额法（TTM 已除权）** | ✓ 分子分母同一时间截面，不受转送股/除权除息影响；只算已落袋分红，不含预案 |
...
**Q: 为什么用 TTM（近12个月已除权）而不是最新完整财年？**
A: TTM 只统计已实施除权的分红（分子分母同一时间截面，不含「预披露」预案），能实时反映最近 12 个月内实际落袋的现金分红，且自动跨财年滚动，避免完整财年法下"年报已出但尚未除权则整体缺失"以及半年报分红被丢弃的问题。
