...
```
真实股息率 = 近12个月已除权现金分红总额 / 当前总市值 × 100%
```
...
只统计**已实施除权**（除权除息日落在近12个月内）的现金分红，排除「预披露」预案：
- TTM 分子分母来自同一时间截面，跨财年自动滚动，不受转送股/除权除息影响
- 相比「最新完整财年」：不会出现年报已出但尚未除权导致整体缺失，也不会丢弃半年报分红（9-12月除权）
- 新年报除权后分子自动切换
